...
void Game::print_manual()
{
    ...
    cout << ""
            "          \n"
            " Move left:    " << CMD_LEFT << "  or  Arrow Left\n"
            " Move right:   " << CMD_RIGHT << "  or  Arrow Right\n"
            " Move up:      " << CMD_UP  << "  or  Arrow Up\n"
            " Move down:    " << CMD_DOWN << "  or  Arrow Down\n"
            "\n"
...
void Game::run()
{
    ...
    // Check and update high score at end of game
    bool new_record = hs.update(s.get_score());
    ...
    cout << "Final Score: " << s.get_score() << "\n";
    if (new_record) {
        cout << "*** NEW HIGH SCORE: " << hs.get() << " ***\n";
    } else {
        cout << "High Score: " << hs.get() << "\n";
    }
}
...
void Game::play_level()
{
    ...
    while (s.is_alive() && !level_over){
        ...
        level_over = !(s.get_dots() < DOTS_PER_LEVEL * level);
        }
        if (s.is_alive())   
            s.place_on_board(board);
        screen_home();
...
void Game::complete_level()
{
    ...
    // Update high score live as levels are completed
    hs.update(s.get_score());
    level++;
