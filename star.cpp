...
void Star::set_num_moves(int n)
{
    num_moves = n;
}
// Move the star
// Supports both WASD keys and arrow keys (ARROW_UP/DOWN/LEFT/RIGHT from termfuncs.h)
bool Star::move(char board[ROWS][COLS], char command)
{
          bool ate_dot = false;

          // Normalize arrow keys to their WASD equivalents
          char effective_cmd = command;
          if (command == ARROW_UP)    effective_cmd = CMD_UP;
          else if (command == ARROW_DOWN)  effective_cmd = CMD_DOWN;
          else if (command == ARROW_LEFT)  effective_cmd = CMD_LEFT;
          else if (command == ARROW_RIGHT) effective_cmd = CMD_RIGHT;

          if (!boulder_ahead(board, effective_cmd)){
                board[row][col] = SPACE;
                if (effective_cmd == CMD_UP){
                        if (row == 0) {row = ROWS-1;} else {row--;}
                        num_moves++;
                } else if (effective_cmd == CMD_DOWN){
                        if (row == ROWS-1) {row = 0;} else {row++;}
                        num_moves++;
                } else if (effective_cmd == CMD_RIGHT){
                        if (col == COLS-1) {col = 0;} else {col++;}
                        num_moves++;
                } else if (effective_cmd == CMD_LEFT){
...
// Check for boulder ahead (uses effective WASD direction)
